#######################################
# Paleoecological Resilience Indicator
# From Stegner, Ratajczak, Carpenter, & Williams (2019) Inferring critical transitions in paleoecological time series with irregular sampling and variable time-averaging. Quaternary Science Reviews 207: 49-63
...
Xct<-single_run(r=r, gens=gens, delta_t=delta_t, K_Start=K_Start, K_Pulse_amt=K_Pulse_amt, V0, pulse_time=pulse_time, driver_press_topo="gradual", q=5)
# plot(Xct[,1],Xct[,3],type="l")

# write.csv(Xct,"GraduallyForcedCriticalTransition.csv",row.names=F)

# run simulations for gradually-forced non-critical transitions
Xdc<-single_run(r=r, gens=gens, delta_t=delta_t, K_Start=K_Start, K_Pulse_amt=K_Pulse_amt, V0, pulse_time=pulse_time, driver_press_topo="gradual", q=1)

# plot(Xdc[,1],Xdc[,3],type="l")

# write.csv(Xdc,"GraduallyForcedNonCriticalTransition.csv",row.names=F)

# run simulations for abruptly-forced critical transitions
Xrs<-single_run(r=r, gens=gens, delta_t=delta_t, K_Start=K_Start, K_Pulse_amt=K_Pulse_amt, V0, pulse_time=pulse_time, driver_press_topo="abrupt", q=5)

# plot(Xrs[,1],Xrs[,3],type="l")

# write.csv(Xrs,"AbruptlyForcedCriticalTransition.csv",row.names=F)

# run simulations for no change scenario
Xnc<-single_run(r=r, gens=gens, delta_t=delta_t, K_Start=K_Start, K_Pulse_amt=0, V0, pulse_time=pulse_time, driver_press_topo="abrupt", q=5)

# plot(Xnc[,1],Xnc[,3],type="l")

# write.csv(Xnc,"NoChange.csv",row.names=F)
